...
paramForExport = "(rw,sync,no_subtree_check)"
...
def setup():
	...
	try:
		...
		log1.info("Return code of 'sudo cp /dev/null /etc/exports on a server: %s", returnCode3)
		#Check a return code
		if (returnCode3 != 0):
			log1.error("A command 'sudo cp /dev/null /etc/exports' on a server failed")

	#OSError exception is raised when a system function returns a system-related error, including I/O failures such as “file not found” or 		“disk full” (not for illegal argument types or other incidental errors).	
	except OSError:
		log1.error("A command 'sudo cp /dev/null /etc/exports' failed. OSError")

	#ValueError raised when an operation or function receives an argument that has the right type but an inappropriate value
	except ValueError:
		log1.error("A command 'sudo cp /dev/null /etc/exports' failed. ValueError")

	log1.info("/etc/exports was successfully cleared")

	#4. On a server assemble a name of a directory for export, a clientIP value and necessary options into a string variable called permForExport
...
#5. On a server create a fileTest file
	try:
		#Execute a child program in a new process
		...
		log1.info("Return code of 'sudo touch /home/dirForTest/fileTest' on a server: %s", returnCode4)
		#Check a return code
		if (returnCode4 != 0):
			log1.error("A command 'sudo touch /home/dirForTest/fileTest' on a server failed")

	#OSError exception is raised when a system function returns a system-related error, including I/O failures such as “file not found” or 		“disk full” (not for illegal argument types or other incidental errors).	
	except OSError:
		log1.error("A command 'sudo touch /home/dirForTest/fileTest' failed. OSError")

	#ValueError raised when an operation or function receives an argument that has the right type but an inappropriate value
	except ValueError:
		log1.error("A command 'sudo touch /home/dirForTest/fileTest' failed. ValueError")

#6. On a server create a file called cinema
	try:
		#Execute a child program in a new process
		proc5 = subprocess.Popen(["sudo", "touch", "/home/dirForTest/cinema"], stdin=subprocess.PIPE)
		proc5.communicate()
		#Wait for child process to terminate. Set and return returncode attribute
		returnCode5=proc5.wait()
		log1.info("Return code of 'sudo touch /home/dirForTest/cinema' on a server: %s", returnCode5)
		#Check a return code
		if (returnCode5 !=0):
			log1.error("A command 'sudo touch /home/dirForTest/cinema' on a server failed. Return code is NOT 0")
		
	#OSError exception is raised when a system function returns a system-related error, including I/O failures such as “file not found” or 		“disk full” (not for illegal argument types or other incidental errors).	
	except OSError:
		log1.error("A command 'sudo touch /home/dirForTest/cinema' on a server failed. OSError")
	
	#ValueError raised when an operation or function receives an argument that has the right type but an inappropriate value
	except ValueError:
		log1.error("A command 'sudo touch /home/dirForTest/cinema' on a server failed. ValueError")



#----6.1 On a server change permissions for /home/dirForTest/cinema
	try:
		#Execute a child program in a new process
		proc5 = subprocess.Popen(["sudo", "chmod", "ugo+rwx", "/home/dirForTest/cinema"], stdin=subprocess.PIPE)
		proc5.communicate()
		#Wait for child process to terminate. Set and return returncode attribute
		returnCode5=proc5.wait()
		log1.info("Return code of 'sudo chmod ugo+rwx /home/dirForTest/cinema' on a server: %s", returnCode5)
		#Check a return code
		if (returnCode5 !=0):
			log1.error("A command 'sudo chmod ugo+rwx /home/dirForTest/cinema' on a server failed. Return code is NOT 0")
		
	#OSError exception is raised when a system function returns a system-related error, including I/O failures such as “file not found” or 		“disk full” (not for illegal argument types or other incidental errors).	
	except OSError:
		log1.error("A command 'sudo chmod ugo+rwx /home/dirForTest/cinema' on a server failed. OSError")
	
	#ValueError raised when an operation or function receives an argument that has the right type but an inappropriate value
	except ValueError:
		log1.error("A command 'sudo chmod ugo+rwx /home/dirForTest/cinema' on a server failed. ValueError")
...
#9. cp /home/dirForTest/fileWithRights to /etc/exports
	...
	try:
		...
		log1.info("Return code of 'sudo cp /home/dirForTest/fileWithRights /etc/exports' on a server: %s", returnCode8)
		#Check a return code
		if (returnCode8 !=0):
			log1.error("A command 'sudo cp /home/dirForTest/fileWithRights /etc/exports' on a server failed. Return code is NOT 0")

	#OSError exception is raised when a system function returns a system-related error, including I/O failures such as “file not found” or 		“disk full” (not for illegal argument types or other incidental errors).	
	except OSError:
		log1.error("A command 'sudo cp /home/dirForTest/fileWithRights /etc/exports' on a server failed. OSError")

	#ValueError raised when an operation or function receives an argument that has the right type but an inappropriate value
	except ValueError:
		log1.error("A command 'sudo cp /home/dirForTest/fileWithRights /etc/exports' on a server failed. ValueError")
...
#12. Execute a command on the SSH server. A new Channel is opened and the requested command is executed. The command’s input and output streams are returned as Python file-like objects representing stdin, stdout, and stderr.
	stdin, stdout, stderr = client.exec_command("echo %s | sudo -S mv /home/$USER/dirForMount/cinema /home/$USER/dirForMount/theatre" % (clientPassword))
	log1.info("End rename a file")
	time.sleep(120)
	log1.info("%s", stderr.read())	
	...
	#Return the exit status from the process on the server. This is mostly useful for retrieving the results of an exec_command. If the command hasn’t finished yet, this method will wait until it does, or until the channel is closed. If no exit status is provided by the server, -1 is returned.
	if stdout.channel.recv_exit_status() != 0:
		log1.error("A command 'echo %s | sudo -S mv /home/$USER/dirForMount/cinema /home/$USER/dirForMount/theatre" % (clientPassword)' on a client failed. The exit status of a command is NOT 0")	
	else:
		log1.info("A command 'echo %s | sudo -S mv /home/$USER/dirForMount/cinema /home/$USER/dirForMount/theatre" % (clientPassword)' on a client passed")

	log1.info("Exit status of 'echo %s | sudo -S mv /home/$USER/dirForMount/cinema /home/$USER/dirForMount/theatre" % (clientPassword)': %s ", stdout.channel.recv_exit_status())
	#Close this SSHClient and its underlying Transport. An SSH Transport attaches to a stream (usually a socket), negotiates an encrypted session, authenticates, and then creates stream tunnels, called channels, across the session
	client.close()
	log1.info("connect with client closed")
...
#13. Execute a command on the SSH server. A new Channel is opened and the requested command is executed. The command’s input and output streams are returned as Python file-like objects representing stdin, stdout, and stderr.
	...
	#Return the exit status from the process on the server. This is mostly useful for retrieving the results of an exec_command. If the command hasn’t finished yet, this method will wait until it does, or until the channel is closed. If no exit status is provided by the server, -1 is returned.
	if stdout.channel.recv_exit_status() != 0:
		log1.info("A command 'echo %s | sudo -S umount /home/$USER/dirForMount/" % (clientPassword)' failed. The exit status of a command is NOT 0. It is: %s", stdout.channel.recv_exit_status())	
	else:
...
#14. Delete /home/dirForTest/' on a server
	try:
		...
		log1.info("Return code of 'sudo rm -rf /home/dirForTest/' on a server: %s", returnCode10)
		#Check a return code
		if (returnCode10 != 0):
			log1.error("A command 'sudo rm -rf /home/dirForTest/' on a server failed. The exit status of a command is NOT 0")

	#OSError exception is raised when a system function returns a system-related error, including I/O failures such as “file not found” or 		“disk full” (not for illegal argument types or other incidental errors).	
	except OSError:
		log1.error("A command 'sudo rm -rf /home/dirForTest/' on a server failed. OSError")

	#ValueError raised when an operation or function receives an argument that has the right type but an inappropriate value
	except ValueError:
		log1.error("A command 'sudo rm -rf /home/dirForTest/' on a server failed. ValueError")	
